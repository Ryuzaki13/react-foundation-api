...
Внешние пакеты, которые этот пакет импортирует напрямую, объявлены как optional peers. Это означает только то, что их не нужно ставить для каждого сценария. Дополнительные зависимости entrypoint, поступающие через `@ryuzaki13/react-foundation-lib`, определяются peer-контрактом этого нижнего слоя.
